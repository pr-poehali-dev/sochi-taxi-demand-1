...
import { fetchAirportData } from "@/lib/api";

const navItems = [
  { id: "map",           label: "Карта",       icon: "MapPin"       },
  { id: "flights",       label: "Рейсы",        icon: "PlaneLanding" },
...
export default function Index() {
  ...
  const [topCoeff, setTopCoeff] = useState("—");
  const [topZoneName, setTopZoneName] = useState("...");
  const [flightCount, setFlightCount] = useState<number | null>(null);

  useEffect(() => {
    fetchAirportData().then(d => {
      const top = d.demand_zones.reduce((a, b) => b.surge > a.surge ? b : a, d.demand_zones[0]);
      if (top) { setTopCoeff(`×${top.surge}`); setTopZoneName(top.name); }
      setFlightCount(d.arrivals.filter(f => f.status !== "arrived").length);
    }).catch(() => {});
  }, []);
    ...
    <div className="min-h-screen bg-background flex flex-col font-ibm">
      ...
      <header className="surface-1 border-b border-subtle px-4 py-3 flex items-center justify-between shrink-0">
        ...
        <div className="hidden md:flex items-center gap-6">
          ...
          <div className="flex items-center gap-2 text-xs cursor-pointer" onClick={() => setActivePage("map")}>
            <Icon name="Zap" fallback="Circle" size={12} className="text-amber-400" />
            <span className="text-foreground font-mono font-semibold">{topCoeff}</span>
            <span className="text-muted-foreground">{topZoneName}</span>
          </div>
          <div className="flex items-center gap-2 text-xs" onClick={() => setActivePage("analytics")} role="button">
      ...
      <div className="flex flex-1 min-h-0">
        ...
        <nav className="hidden md:flex flex-col surface-1 border-r border-subtle w-52 shrink-0 py-3">
          <div className="space-y-0.5 px-2">
            {navItems.map(item => {
              const badge = item.id === "flights" && flightCount !== null ? String(flightCount) :
                ...
                >
                  ...
                  {badge && (
                    <span className={`text-xs font-mono px-1.5 py-0.5 rounded-full ${
                      activePage === item.id ? "bg-blue-500/30 text-blue-300" : "bg-white/10 text-muted-foreground"
                    }`}>{badge}</span>
                  )}
                </button>
              );
          ...
          <div className="mt-auto px-2 pt-3 border-t border-subtle mx-2">
            <div className="surface-2 rounded-lg border border-subtle p-3 cursor-pointer" onClick={() => setActivePage("forecast")}>
      ...
      <nav className="md:hidden surface-1 border-t border-subtle flex items-center justify-around px-1 py-2 shrink-0">
        {navItems.map(item => {
          const badge = item.id === "notifications" ? "2" : null;
          return (
            <button key={item.id} onClick={() => setActivePage(item.id)}
              className={`flex flex-col items-center gap-1 px-3 py-1 rounded transition-all relative ${activePage === item.id ? "text-blue-400" : "text-muted-foreground"}`}
            >
              <Icon name={item.icon} fallback="Circle" size={20} />
              <span className="text-[10px]">{item.label}</span>
              {badge && <span className="absolute -top-0.5 right-1 w-4 h-4 bg-red-500 rounded-full text-[9px] text-white flex items-center justify-center">{badge}</span>}
            </button>
          );
        })}
      </nav>
